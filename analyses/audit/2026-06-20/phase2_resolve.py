#!/usr/bin/env python3
"""Phase 2 (2026-06-20) — C20 daily-bar path-aware resolution + C21 SPY benchmark.

Adapted from the 2026-05-30 resolver. Changes:
  * Weekly entry = report_date directly (weekly envelopes carry the Friday date).
  * Resolve PAPER outcomes for ALL directional/vol rows regardless of tier/size
    (Phase 3 score-vs-outcome scatter + tier reliability need benched-name outcomes);
    carry `was_sized` so the sized-book expectancy view stays separable. Only
    genuinely directionless neutral rows (non-pin) are NOT_A_TRADE.
  * canonical_class: collapse the fragmented dealer*/distribution labels (Phase-1 flag).
"""
import json, os, statistics
from collections import defaultdict

AUD = "analyses/audit/2026-06-20"
OHLC = f"{AUD}/_ohlc"
...
def resolve_directional(bars, ei, direction, nwin):
    ...
    # 3D corroboration (fast vs slow trigger)
...
def main():
    ...
    for r in rows:
        ...
        edate = r["report_date"]  # weekly envelopes carry the Friday date directly
        ...
        if directionless:
            res = {"outcome": "NOT_A_TRADE", "inconclusive_reason": "directionless"}
            mode = "not_a_trade"
        elif tk not in PX:
        ...
        # LEAP never resolved until window closes
    ...
    # decided split by sized vs paper
