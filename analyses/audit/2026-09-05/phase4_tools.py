...
# C16/C62 — 2026-08-30 audit P2 #2: this block previously read
# `fz_context.dp_block_to_float_ratio` and reported "NA — not in envelope" while the
# field was populated. The schema and BOTH emitters write it at the CALL level:
# `decision.json.calls[].dp_block_to_float_ratio`. Any prior-cycle "not emitted" claim
# sourced from this helper is unverified — re-derive it from the schema path.
_fr = [r for r in rows if r.get("dp_block_to_float_ratio") is not None]
_frd = [r for r in _fr if r["outcome"] in ("WIN", "LOSS")]
_C62_THRESHOLD = 0.0010  # PRE-REGISTERED 2026-08-22 — never re-fit; a re-fit voids C62
_above = [r for r in _frd if r["dp_block_to_float_ratio"] >= _C62_THRESHOLD]
_below = [r for r in _frd if r["dp_block_to_float_ratio"] < _C62_THRESHOLD]
_wr = lambda x: round(sum(1 for r in x if r["outcome"] == "WIN") / len(x), 3) if x else None
fz["C16_float_normalized_block"] = {
    "populated": len(_fr), "decided": len(_frd),
    "threshold": _C62_THRESHOLD,
    "n_above": len(_above), "wr_above": _wr(_above),
    "n_below": len(_below), "wr_below": _wr(_below),
    "regime_buckets": len({r["regime_bucket"] for r in _frd}),
    "bar": "n>=30 decided, >=10 per arm, >=2 regime buckets, BH-surviving, threshold FIXED",
    "verdict": ("OPEN — %d/30 decided, %d/10 in the upper arm" % (len(_frd), len(_above))
                if (len(_frd) < 30 or len(_above) < 10) else "BAR MET — grade for promotion"),
}
