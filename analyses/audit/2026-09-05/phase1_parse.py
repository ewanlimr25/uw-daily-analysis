...
for f in files:
    ...
    for c in d.get("calls", []):
        ...
        row = {
            ...
            # 2026-08-30 audit P2 #2 — ROOT CAUSE of the "C16 not in envelope" misreport.
            # The instrumentation trio lives at the CALL level per the schema, but this
            # parser never carried it into the JSONL, so every downstream helper looked in
            # fz_context, found nothing, and reported the field as unemitted. It was emitted.
            "dp_block_to_float_ratio": c.get("dp_block_to_float_ratio"),
            "implied_move": c.get("implied_move"),
            "insider_cluster_flag": c.get("insider_cluster_flag"),
            # 2026-08-30 audit P1 #1 — new; null on every pre-2026-08-30 envelope by design.
            "fundamentals_verdict_reason": c.get("fundamentals_verdict_reason"),
