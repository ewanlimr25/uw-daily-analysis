...
def canonical_class(c):
    ...
    # 2026-08-22 P1 #2: the schema now declares vol_long/vol_short as ALIASES of the
    # canonical class vol_term_dislocation. Apply the subject's own alias map so the
    # 5 drifted 2026-08-18 rows rejoin their denominator instead of sitting below N>=8.
    if cl in ("vol_long", "vol_short"):
        return "vol_term_dislocation"
    return c
...
def resolve_pin(bars, ei, nwin=5):
    ...
    C59 DECIDED 2026-08-30 -> C64 APPLIED. The bar was >=3 of 9 rows diverging; measured
    5 of 11. TOUCH scored 2/11 (18.2%), SETTLEMENT 7/11 (63.6%) -- a 45.4pp swing on
    identical rows, with four of the five divergent names settling within 0.21% of entry
    after brushing a wing intraday. An iron fly / short straddle / butterfly pays on where
    price FINISHES, so SETTLEMENT is the correct rule and is now the headline `outcome`.

    The legacy TOUCH rule is retained as `outcome_touch` for continuity with the prior
    twelve audits. NOTE FOR WHOEVER READS A PRE-2026-08-30 opex_pin FIGURE: those numbers
    were computed under TOUCH and are SUPERSEDED, not comparable -- do not chart them
    against post-change values.
    """
    R = atr_true_range(bars, ei)
    if R is None or R <= 0:
        return None
    ...
    if not fwd:
        return {"outcome": "INCONCLUSIVE", "inconclusive_reason": "window_open", "entry_px": entry,
                ...
                "max_adverse_excursion_pct": None, "trigger_offset": None,
                "ambiguous_bar": False, "window_complete": False, "nwin": nwin,
                "pin_rule": "settlement", "outcome_touch": "INCONCLUSIVE",
                "pin_rule_divergent": False}
    ...
    # C64 (2026-08-30): SETTLEMENT is the headline; TOUCH is retained for continuity.
    return {"outcome": outcome_settle,
            "inconclusive_reason": None if outcome_settle != "INCONCLUSIVE" else "window_open",
            "trigger_offset": None,
            "realised_return_pct": round((R - settle_dist) / entry * 100, 3),
            ...
            "pin_rule": "settlement",
            "outcome_touch": outcome,
            "touch_realised_return_pct": round((R - maxexc) / entry * 100, 3),
