...
AUDIT = "analyses/audit/2026-09-05"
P0 = "2026-08-03"
...
# ============================================================================
# 2026-08-30 audit P1 #3 — SHORT-THESIS GENERATION RATE (standing measurement)
# ============================================================================
# The 2026-08-01 P0 routes shorts to watch_only. It is falsifiable ONLY through
# resolved short rows, so generation rate is a first-class compliance metric, not
# colour. 2026-08-22 measured a HALVING (32.0% -> 17.6%); the 08-22 P1 #1 generation
# floor reversed it (-> 44.2%, Fisher p=0.0003). The NEXT failure mode is the mirror:
# a floor becoming a quota. Post-fix uptrend share already reads 60.0% vs a 31.3%
# pre-P0 baseline (p=0.0422, n=15). Report both directions every cycle; act on neither
...
def _fisher(a, b, c, d):
    ...
    def lc(n, k):
        return None if k < 0 or k > n else _math.lgamma(n + 1) - _math.lgamma(k + 1) - _math.lgamma(n - k + 1)
    n, r1, r2, c1 = a + b + c + d, a + b, c + d, a + c

    def pr(x):
        t1, t2 = lc(r1, x), lc(r2, c1 - x)
        return 0.0 if t1 is None or t2 is None else _math.exp(t1 + t2 - lc(n, c1))
    p0 = pr(a)
    return min(1.0, sum(pr(x) for x in range(max(0, c1 - r2), min(r1, c1) + 1) if pr(x) <= p0 * (1 + 1e-9)))


def short_generation_table(rows, boundaries):
    """boundaries: ordered [(label, lo_date_inclusive, hi_date_exclusive), ...]"""
    out = []
    for lab, lo, hi in boundaries:
        for rb in (None, "uptrend"):
            sub = [r for r in rows
                   if (lo is None or r["report_date"] >= lo)
                   and (hi is None or r["report_date"] < hi)
                   and (rb is None or r.get("regime_bucket") == rb)]
            if not sub:
                continue
            sh = sum(1 for r in sub if r.get("thesis_direction") == "short")
            out.append({"period": lab, "regime": rb or "ALL", "shorts": sh,
                        "n": len(sub), "share": round(sh / len(sub), 4)})
    return out


PERIODS = [("pre-P0", None, "2026-08-03"),
           ("post-P0 pre-floor", "2026-08-03", "2026-08-22"),
           ("post-floor", "2026-08-22", None)]

print()
print("=" * 78)
print("P1 #3 (2026-08-30) — SHORT GENERATION RATE: floor vs quota")
print("=" * 78)
_tab = short_generation_table(rows, PERIODS)
for t in _tab:
    print(f"  {t['period']:20} {t['regime']:8} {t['shorts']:4}/{t['n']:4} = {t['share']*100:5.1f}%")
for rb in ("ALL", "uptrend"):
    g = {t["period"]: t for t in _tab if t["regime"] == rb}
    if len(g) == 3:
        a, b = g["post-P0 pre-floor"], g["post-floor"]
        c = g["pre-P0"]
        print(f"  [{rb}] pre-floor vs post-floor Fisher p={_fisher(a['shorts'], a['n']-a['shorts'], b['shorts'], b['n']-b['shorts']):.4f}"
              f"  | pre-P0 vs post-floor p={_fisher(c['shorts'], c['n']-c['shorts'], b['shorts'], b['n']-b['shorts']):.4f}")
print("  READ: a share ABOVE the pre-P0 baseline is the quota failure mode; a share")
print("        materially BELOW it is the starvation failure mode. Both are drift.")
