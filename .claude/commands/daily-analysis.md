...
6. **Top-of-funnel screens** — run in parallel:
   - `uw screener bullish-bearish` (top 25 each side) — net premium leaderboard.
   - `uw insights signal-confluence` (`min_score=3`, top 25 each direction) — multi-factor scoring; agents start their hunts here.
   - `uw screener volume-vs-average` (top 25, `--min-volume-ratio 3`) — flow anomalies vs 30-day baseline.
   - `uw screener iv-rank` (top 25 high, top 25 low) — premium-selling and premium-buying candidates.
...
- `uw screener iv-rank` — extremes for ranking.
...
2. **Emit the structured decision envelope** beside the report at `analyses/daily/YYYY-MM-DD/decision.json`, conforming to `schemas/decision_envelope.schema.json`. This is the machine-resolvable sidecar `/calibration-audit` Phase 1 reads instead of re-parsing prose. Build it from the data already produced — do not re-derive:
   - Top level: `{schema_version: "1.1", report_date, report_kind: "daily", regime, vrp_classification, macro_snapshot_signals (the fred_macro signals object), macro_event_risk (the event_risk calendar), watchlist_write_back (the persisted top-5), next_session_gex, next_session_0dte_setup, breadth_cross_check, report_path}`.
