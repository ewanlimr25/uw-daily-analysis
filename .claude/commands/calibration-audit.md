...
> **Method register (2026-05-30 meta-audit, `analyses/audit/2026-05-30/calibration_meta_audit.md`).** Auditor-method fixes applied: **C20** daily-OHLC path-aware resolution (replaces the non-executable `uw historical trend --date`/high-low spec; OHLC comes from the Yahoo **chart API** the yfinance MCP wraps — the `mcp__yahoo-finance__*` tools themselves return close-only and cannot resolve the path rule); **C21** benchmark-excess (realised − SPY same-window) as a first-class Phase-3 column — the audit now measures *edge*, not just calibration; **C22** removes look-ahead from the truth-set (`signal-backtest` has no `--date`); **C23** decided-N≥8 floor + Benjamini-Hochberg across flagged class/tool stats + reconstructed-citation priority cap; **C24** Phase-6 gate *effectiveness* (downgrade-effectiveness, VETO false-positive rate) on top of gate firing; **C25** reliability diagram + log-loss beside Brier. These are *method* changes to the auditor; the audit it runs stays propose-only and never edits the subjects.
>
> **Method register addendum (2026-07-25 audit, `analyses/audit/2026-07-25/`).** Two further *auditor* fixes — the subject rubric freeze is untouched by both: **C49** demotes raw benchmark-excess from headline statistic to an annotated secondary column and promotes **tape-conditioned book WR + paired McNemar** to the primary edge test (Phase 3d measured `excess_pp = +36.5 − 80.0 × spy_wr`, R² 0.636 — **71.1% of the excess column's variance is the benchmark moving**, and three consecutive audit headlines were substantially artifacts of the denominator); **C54** excludes `tier == DROP` rows from missed-gate denominators (Phase 6.5: 0/7 HIGH, 0/20 MEDIUM, 0/94 LOW, 211/374 DROP — the aggregate "34% drift" was the fleet's early-exit optimization being scored as non-compliance, and it mis-flagged `risk-monitor.md` for drift that does not exist).
...
**Resolution granularity — daily OHLC, not close-only (C20).** The path rule is only real if you can see the intra-window low (for a long) or high (for a short). Resolve on **daily OHLC bars** pulled from the **Yahoo chart API the yfinance MCP wraps** — `https://query1.finance.yahoo.com/v8/finance/chart/<sym>?period1=<epoch>&period2=<epoch>&interval=1d`, reachable directly via stdlib `urllib`. Its `chart.result[0].indicators.quote[0]` carries real `open/high/low/close` per day (verified 2026-05-30; its `close` matches the MCP exactly). **Do NOT use the `mcp__yahoo-finance__*` tools to resolve outcomes** — all three (`get_historical_stock_prices`, `get_stock_price_date_range`, `get_stock_price_by_date`) flatten to **close-only** (`{date: close}`) in this environment and therefore cannot evaluate the path rule. (Only intraday-*tick* range is unavailable; daily high/low **is** available via the chart API.) The canonical fetcher is `analyses/audit/2026-05-30/fetch_ohlc.py` (epoch window, retry, real-OHLC sanity check). Compute `ATR(14)` as **true range** — `max(high−low, |high−prev_close|, |low−prev_close|)` averaged over the 14 bars before entry — not the close-to-close range proxy. Then walk the window bar-by-bar: a long is a LOSS the first day `low` breaches `entry − R` *before* any day's `high` reaches `entry + R`; symmetric for a short. This recovers ~80% of the true path rule. Record `max_adverse_excursion_pct` from the daily extreme, not the daily close. **Do not** silently degrade to close-to-close — if a ticker has no daily OHLC, tag it INCONCLUSIVE (`data_unavailable`), never a LOSS. The earlier "close-only is the only option" framing was wrong: daily high/low is available and the path rule must use it.

### Tool calls

For each row in Phase 1:

1. **Price window (the WIN/LOSS resolver) — daily OHLC.** Pull the **Yahoo chart API** (`query1.finance.yahoo.com/v8/finance/chart/<sym>`, stdlib `urllib` — **not** the close-only `mcp__yahoo-finance__*` tools; see the C20 granularity note above) for `ticker`, spanning `report_date` through `report_date + horizon_window` (trading days). Capture per-bar `high`/`low`/`close`; compute entry = the `close` on `report_date` (or next session if the report is post-close), `R` = 0.5 × true-range-ATR(14) at entry, intra-window MAE from the daily extremes, and end-of-window close. **`uw historical trend` is NOT a price-window tool** — it has no `--date` flag and returns options-flow metrics + daily close only (no high/low). Use it, if at all, only to read the *flow context* at entry, never to resolve outcomes. (Earlier versions of this skill specified `uw historical trend --date … capture intra-window high/low`; that method is not executable — the flags and fields don't exist. Do not reintroduce it.)
...
1. **Per-signal-class table.** For each `dominant_signal_class` with **decided-N ≥ 8** in the dataset (C23 — raised from 5; a divergence on n=5–7 is noise, and the 2026-05-29 run tabled −40pp on n=1 claims that should never have reached a reader):
   ...
   - **PRIMARY EDGE TEST — tape-conditioned book WR + paired McNemar (C49, 2026-07-25).** Split the class's decided rows by the **realised tape** of their own resolution window (UP / DOWN, from the same-window SPY path) and report **book WR per tape arm** with its `n`, plus `|Δ book WR|` between arms. Then run a **paired McNemar** test on the row-matched pairs (each row's own outcome against its own `spy_benchmark_win` over the *identical* window) and report the McNemar statistic, p-value, and the discordant-pair counts. **This pair — tape-conditioned book WR and row-matched McNemar — is what a claim of edge must rest on.** McNemar is the right test precisely because it is row-matched: the benchmark cancels pairwise instead of entering as a stratum-level denominator.
   - **SECONDARY — realised benchmark-excess (C21, DEMOTED by C49).** `realised_excess` = realised WR − SPY same-window same-direction WR, via `scripts/excess_winrate.py:market_excess(realised_wr, spy_benchmark_wr)`, where `spy_benchmark_wr` = the fraction of that class's rows whose `spy_benchmark_win` (Phase 2) fired. It remains the right *concept* — a long book winning 45% where SPY-long wins 28% has done something — but it may **never be printed alone and never lead a finding**. Three mandatory companions on every row that carries an excess number:
     1. `spy_benchmark_wr` itself, printed **beside** the excess;
     2. the **dispersion ratio** `sd(book WR) / sd(benchmark WR)` across the strata in the table — a ratio well below 1.0 says the excess column is tracking the denominator;
     3. the tape-conditioned book WRs from the primary test above.
     **Why (Phase 3d, 2026-07-25).** OLS across 34 strata cells: `excess_pp = +36.5 − 80.0 × spy_wr`, R² = **0.636**, implied β(bookWR | spyWR) = **+0.20** — **71.1% of the excess column's variance is the benchmark moving, 28.9% is the book**. Robust across n≥15, n≥25, extreme-cell exclusion and n-weighting (slope −73 to −88). Three of four canonical classes had a book WR that did **not move at all** between rising and falling tape (`bearish_flow` 0.533→0.526, `dark_pool_accumulation` 0.400→0.400, `bullish_flow` 0.400→0.429) while their excess column swung 9–23pp. The excess statistic amplifies stratum noise ~3.1×. The 2026-07-11 "+29.4pp `bearish_flow` edge" headline and the 2026-07-18 "the sign reversed" headline were **substantially the same rows with a different denominator**. Do **not** delete the column — the paired long-book result survives precisely because McNemar is row-matched — but a single-window excess number quoted *as a finding about a signal class* is not defensible.
   - **Calibrated-but-beta flag (retained, re-anchored).** Flag a class as **"calibrated-but-beta"** only when calibration ✅ **and** the paired McNemar is non-significant **and** excess ≤ 0 — i.e. the row-matched test agrees with the excess column. An excess ≤ 0 with **no** McNemar support is a *denominator* reading, not a class finding, and must be reported as such.
   - N (decided)
   ...
   - **Do not re-derive the artifact each cycle.** When a class's *excess* sign flips versus the prior audit but its *book WR* does not move materially between the two windows, the correct write-up is "the benchmark moved," not "the edge reversed." Check the book column before narrating a reversal — this failure has now cost three consecutive headline findings.
...
- `phase_3_calibration.md` — the desk reads the class table (**led by the tape-conditioned book WR + paired-McNemar columns; benchmark-excess sits to their right, annotated**), the per-tier diagram, the **reliability deciles**, Brier + **log-loss**, and a written verdict. The verdict has **four** sections: (a) "where the rubric is honest," (b) "where it lies to itself," (c) "tier inversions or overconfidence on High-tier," and **(d) "calibrated-but-beta" — classes whose claimed≈realised, whose paired McNemar is non-significant, AND whose benchmark-excess ≤ 0 (they ride the tape, they don't beat it).** (d) is the desk-critical read: a class can pass calibration and still be worthless edge — but per **C49** it takes the row-matched test, not the excess column alone, to say so.
- **Header requirement (C49).** Print the table-level **dispersion ratio `sd(book WR) / sd(benchmark WR)`** and the OLS of `excess_pp` on `spy_wr` (slope, R²) in the phase header, *before* any class commentary. A slope near −100 and R² above ~0.5 means this cycle's excess column is again mostly denominator — say so in the header and calibrate every excess sentence below it accordingly.
- `phase_3_calibration.jsonl` — per-signal-class numerics incl. `book_wr_up_tape` / `book_wr_down_tape` / `n_up` / `n_down` / `abs_delta_book_wr`, `mcnemar_stat` / `mcnemar_p` / `discordant_b` / `discordant_c`, `realised_excess`, `spy_benchmark_wr`, `dispersion_ratio`, BH-adjusted p-values, the reliability-decile array, and per-tier + log-loss/Brier scalars.
...
1. **Quant compliance**:
   ...
   - Backtest sizing-map honored? Check against the **live** subject map (`signal-confluence-quant.md`): **≥0.70 → full, 0.65–0.70 → half, [0.55,0.65) → starter (anti-predictive band floor, 2026-07-25 P1 #4), 0.50–0.55 → half, <0.50 → starter/skip** (the full-size line was tightened 0.65→0.70 by the 2026-05-23 register; auditing against the old 0.65 mis-flags compliant half-sizes. A pre-2026-07-25 row quoting in [0.55,0.65) at `half` is **era-correct**, not a violation — grade it against the map live on its `report_date`). The size vocabulary the rubric actually emits is **`full | half | quarter | starter | skip`** — `quarter` sits between half and starter; include it in the order map so a legitimate `quarter` downgrade is not flagged as a violation (this caused several false positives in the 2026-05-29 run). Gate-driven downgrades below the map size are always allowed; only an *upgrade* above the win-rate-implied size is a violation.
...
- **Denominator rule — DROP rows are excluded (C54, 2026-07-25).** The missed-gate **rate** is computed over **non-DROP rows only** (`tier ∈ {HIGH, MEDIUM, LOW}` plus `watch_only`). DROP names are eliminated *before* the full risk stack runs — that is the fleet's intended early-exit optimization, and counting it as non-compliance measures efficiency as drift. Report the ledger **decomposed per tier** (HIGH / MEDIUM / LOW / DROP), and keep the **DROP figure visible as its own line** so a genuine future drift confined to DROP rows is not masked — it is simply never folded into the headline rate or the 20% drift threshold. **Evidence (Phase 6.5, 2026-07-25):** raw aggregate rates read regime 34.1%, vrp 35.1%, event_risk 32.7%, fundamentals 33.4% — all past the drift threshold. Decomposed: regime **0/7 HIGH, 0/20 MEDIUM, 0/94 LOW, 211/374 DROP**; vrp 0/7, 0/20, 1/94, 215/374; event_risk 0/7, 0/20, 0/94, 206/374; fundamentals 0/7, 0/20, 2/94, 207/374. **Three misses across 121 non-DROP rows.** The aggregate had been mis-flagging `risk-monitor.md` for agent drift that does not exist.
...
If the **non-DROP** missed-gate rate for any specific gate exceeds 20% (the C54 denominator above — never the DROP-inclusive aggregate), that's evidence the agent's prompt and its realised behavior have diverged. Identify which agent file needs patching (`risk-monitor.md` for risk gates; `signal-confluence-quant.md` for scoring math). A DROP-only elevation is reported as a separate observation and does **not** trigger a drift finding or an agent-file recommendation.

### Output

- `phase_6_decision_audit.md` — compliance rates, missed-gate ledger (**per-tier decomposition, non-DROP headline rate, DROP on its own line — C54**), **gate-effectiveness table (downgrade-effectiveness per gate + `veto_fp_rate` + debate-gate effectiveness, C24)**, per-agent drift findings. Voice: buy-side PM doing post-mortem on a fund — clinical, specific, names the agent file. State the decided-N behind every effectiveness number; mark `< 10` as advisory.
...
- **Edge before calibration in the verdict — but the edge claim must be row-matched (C21 as amended by C49).** When a class is both miscalibrated (Phase 3 claimed-vs-realised) *and* carries an edge problem, lead with the **edge** problem — a desk loses money on negative edge long before it loses on an honest-but-wrong win-rate quote. **What counts as an edge finding is now specified:** tape-conditioned book WR and/or a significant paired McNemar. A **raw benchmark-excess number may never be the headline of a recommendation**, may never be quoted without its `spy_benchmark_wr` and the table's dispersion ratio beside it, and may never by itself justify a priority above **P2**. Grounds (Phase 3d, 2026-07-25): 71.1% of the excess column's variance is the benchmark; the 2026-07-11 and 2026-07-18 headline findings — and their apparent mutual contradiction — were the same rows under a moved denominator. A finding whose *book* win rate did not move between windows is not a finding about the book.
