...
Run a full weekly intelligence note in the voice of a top-tier institutional desk strategist. Uses the same agent fleet as `/daily-analysis` but wired to week-range inputs and a persistence-weighted conviction rubric. **All data is pulled fresh via the `uw` CLI — no dependency on prior daily analysis files** (one deliberate exception: the Step 6 thesis scorecard reads the week's daily `decision.json` envelopes — the only hindsight-free record of what was called — see Step 6, 2026-06-12 P2.1). Phase 1 spawns **11 agents** (12 in OPEX week) in parallel against a shared week-baseline context (including a FRED macro snapshot + forward event-risk calendar). Phase 2 runs `signal-confluence-quant` for audited scoring, then a `fundamentals-gate` cross-check and a bounded `bull-researcher`/`bear-researcher` debate on the top-5, then `risk-monitor` for gating, against the union of the week's candidates. Score conviction formally with explicit tiers, backtest and fundamentally vet top names, write winners back to the watchlist, emit a machine-readable `decision.json` envelope, and save the report to its own folder `analyses/weekly/YYYY-WW/` holding `report.md` + `decision.json`.
...
**Models and effort are pinned per-agent in `.claude/agents/*.md` frontmatter (`model:` + `effort:`) — that frontmatter is the single source of truth; do NOT restate model assignments in spawn prompts** (prompt-header "Model:" lines are mechanically inert — they never change the spawned model; a stale table of them lived here until the 2026-07-03 audit). Current pins (re-evaluated 2026-08-08 for the Claude 5 generation — Opus 5 / Sonnet 5): `signal-confluence-quant` and `risk-monitor` = opus/high (envelope-writing + state-mutating choke points; formerly fable), the rest of the fleet = sonnet — **high** for synthesis-heavy agents (accumulation-hunter, dealer-positioning-strategist, sector-rotation-strategist, earnings-scout, contrarian-scanner, multileg-strategist, bull/bear-researcher — debate pair symmetric), **medium** for mechanized/script-driven ones (sweep-tracker, leap-positioning-radar, fundamentals-gate, vol-surface-scout, gamma-flip-tracker, opex-pin-strategist). Original rationale, evidence, and escalation triggers: `analyses/audit/2026-07-03/agent-model-effort-audit.md`; the next `/calibration-audit` must grade this model transition.

**Run the orchestrating session itself on opus minimum (Opus 5 or better)** — Step 0 synthesis, the confluence gate, rubric application, and report + envelope authoring live in the main loop, and the two fable frontmatter pins protect only Phase 2's core, not the orchestration.
...
Weekly conviction score = Σ:
  ...
  +3  uw historical oi-trend BUILDING for the full week, --days ≥ 5 (accumulation-hunter / leap-positioning-radar)   # WEEKLY-ONLY +3 vs DAILY +1 (daily reduced +2→+1 on 2026-05-09 as 'correlated with the LOAD-BEARING components'). Documented divergence (2026-06-12 P1.2): the weekly weight is intentionally higher because BUILDING sustained across a full 5-day covered week is a materially stronger persistence read than a single daily ≥5-day-lookback snapshot — but the same correlated-component concern the daily demote cited applies here too (OI build co-moves with the +3 accumulation conjunction and historically with cum-flow). FROZEN at +3 (P0.1); a candidate over-weight is PRE-REGISTERED for decision at a future cross-regime audit: hypothesis 'weekly OI +3 double-counts with the accumulation conjunction', acceptance bar = cross-regime ∧ n≥30 per arm ∧ BH-surviving, decision window post-2026-06-12 TRANSITIONAL accrual. SHARPENED → register C47 (2026-07-04 audit P2 #3, weight UNCHANGED under the freeze): the backing tool `historical oi-trend` measured −9.2pp class-controlled marginal (n=69, negative in 3 of 4 regime strata; choppy −36.4pp) AND carries a documented measurement artifact — `consecutive_build_days` ceilings at `--days` on liquid names (W27 §7(4) Gate-1 saturation), so the +3 can be earned by expiry mechanics, not accumulation. C47 direction: reduce toward +1 or gate the award on a saturation-aware read (DTE>3-filtered OI, `consecutive_build_days < --days` ceiling check). Acceptance bar: cross-regime ∧ n≥30 per arm ∧ BH-surviving FDR 0.10 on the artifact-corrected read; decision by a FUTURE /calibration-audit — do NOT act on this annotation. Until then, when quoting this line's evidence, state whether `consecutive_build_days == --days` (saturated: the read is a floor, not a count).
...
**SHORT-direction routing — `watch_only`, never sized (2026-08-01 audit P0 #1; register #1 CLEARED).** Applied before the multiplier table above: any call whose `direction == short` routes to **`watch_only`** regardless of tier or `win_rate`, instrument-agnostic (single name or index/ETF). **Routing, not suppression** — §3b still runs, theses/structures/invalidations still print, the full 9-key `gate_verdicts` still records, and the call still serializes into `decision.json` so `/calibration-audit` keeps resolving it. **Out of scope:** short legs inside defined-risk spreads, short-vol structures, and the beta-hedge sleeve. **Evidence:** paired McNemar `ALL / short` **p=0.0115, BH-surviving** (b=24 vs c=46, n=163) — row-matched, so the C49 excess-denominator artifact cannot explain it; both tape arms clear n≥30 and agree (**UP −13.0pp n=92 / DOWN −14.1pp n=71**), i.e. mis-selection not mistiming, which closes the "shorts regain edge in a real downtrend" question the 2026-06-27 ruling left open. The **long** book is untouched (`ALL / long` p=0.0046, BH-surviving, n=204). Enforced at emission by `signal-confluence-quant.md` and again by `risk-monitor.md`.

**GENERATION FLOOR — the routing is terminal sizing, never Phase-1 screening (2026-08-22 audit P1 #1).** Short candidates that clear the Phase-1 entry bar are **scored, gate-verdicted and serialized first**; the `watch_only` routing is applied at the *end*, on the sizing line. Never drop a short at candidate-union time, never filter the short lane out of the Phase-1 fan-out, and never omit a short's `score_components[]` or its 9-key `gate_verdicts` on the grounds that it cannot be sized. **⚠️ This drift is measured, not hypothetical:** the 2026-08-22 audit (Phase 6.5) found compliance with the letter perfect for a third cycle (0 sizing violations, counterfactual preserved 28/28) while short-thesis **generation halved** — short share of the board **32.0% → 17.6%** (Fisher two-sided **p = 0.00035**), and **at a fixed `uptrend` regime bucket still 31.3% → 17.4% (p = 0.0045)**, so regime does not explain it. The routing rule is falsifiable only through resolved short rows: its revisit trigger reads a *paired* McNemar on exactly those rows (`ALL / short` **p = 0.0026**, −14.5pp, BH-surviving, n=193 as of 2026-08-22). Starve the lane and the rule becomes unfalsifiable. **A session that yields zero short candidates in a non-trending tape is a screen to re-check, not a clean board.** **✅ 2026-08-30 audit (Phase 6.5) — THE FLOOR WORKED; now watch the opposite failure.** Short share recovered **17.6% → 44.2%** on the first week under this rule (Fisher two-sided **p = 0.0003**), and **at a fixed `uptrend` regime bucket 17.4% → 60.0% (p = 0.0007)**. Routing compliance stayed perfect a 4th cycle (51 post-P0 shorts: 46 `watch_only`, 4 `skip`, 1 `veto`; **0 sizing violations**; counterfactual intact on every non-DROP short). **New watch item:** post-fix share is now *above* the pre-P0 baseline — 44.2% vs 32.0% overall (p = 0.0896, ns) but **60.0% vs 31.3% at fixed uptrend regime (p = 0.0422)** on n=15. A generation *floor* becoming a generation *quota* is the mirror-image defect. **Do not act on n=15** — re-measure next cycle. Manufacturing shorts to hit a rate is as damaging as starving them.

**SHORT-VOL routing — `watch_only`, never sized (2026-08-30 audit P0 #1).** A **second, independent** routing rule, applied the same way and at the same point as the SHORT-direction rule above: any call whose `direction == vol_short` / `section == vol_short`, **or whose structure is net short vega** (iron condor, iron fly, short straddle or strangle, credit spread sized on premium capture, a front leg sold outright), routes to **`watch_only`** regardless of tier, `raw_score`, `win_rate` or VRP sign. **Routing, not suppression, and terminal sizing rather than screening** — the vol scouts keep proposing SELL VOL, §5 still prints the thesis/structure/invalidation, the full 9-key `gate_verdicts` still records, and the call still serializes so `/calibration-audit` keeps resolving it. **Note the interaction with the scope line above:** the SHORT-direction rule excludes "short legs inside vol structures" because those are not *directional* short alpha — this rule covers the complementary case, a net-short-**vega** expression. A short leg inside a *long*-vol or defined-risk structure (the sold wing of a calendar, the far strike of a debit spread, a hedge sleeve) is covered by neither.

**`vol_long` / BUY VOL is explicitly OUT OF SCOPE and must not be constrained** — it measures **+9.5pp against an unselected same-date single-name peer benchmark** (n=43, paired McNemar **p=0.0034**) and is **5-for-5 on every sized row in the corpus**.

**Evidence.** `vol_short` books **31.5%** against unselected same-date single-name peers at **56.6% — −25.1pp on n=111** strict-window rows, paired McNemar against the peer-median partner **b=2, c=57, p < 0.0001**; negative and **individually significant in 4 of 4 regime buckets** (−20.2 / −35.1 / −23.5 / −25.5pp) and 3 of 4 months. Loose-window −33.6pp on n=134, BH-surviving. Four corroborating instruments: `earnings_vol` claims 0.87 / realises 0.33 (n=144, BH-surviving); `vol_term_structure(+/−)` −13.4pp, **sole BH survivor of 24 component lines**; vol horizon 32.8% vs a 38.5% book; August `vol_short` **8.3%**. **Mechanism: a sign error, not a dead lane** — the scouts identify vol-**expansion** candidates correctly (that is what `vol_long`'s +9.5pp says) and the same identification is used to **sell** vol into the expansion (selected names' mean realised-vol ratio **1.132** vs a peer median **0.962**). **Not the tape:** August was a short-vol paradise at the index (SPY RV ratio 0.738, 12% of windows expanding) while the book's names ran 1.199 — the confound runs opposite to the excuse. **Exposure: `vol_short` is 20 of the 48 sized rows in the entire corpus — 42% of every position this fleet has ever taken.**

**Why routing and not a filter — tested before shipping.** The audit's own P0 first proposed an at-entry expansion veto; it was falsified against the corpus (`analyses/audit/2026-08-30/phase_7b_p0_design_test.md`). The relative-to-SPY leg ran **backwards** (names calmer than SPY realised 13.9%; names >0.15 hotter realised 37.7%), the combined veto was **net −14.3pp**, and **no threshold in [0.70, 1.00] lifted the surviving arm above −21pp vs peers** — every surviving arm kept a McNemar p < 0.0001. **Do not re-propose a threshold veto without new evidence.**

**Caveat that travels with every number here.** Vol rows resolve on an RV-**direction** proxy, not IV-vs-RV, so these figures price **SELECTION, never P&L** — never quote them as a P&L claim. This is why `implied_move` on every vol row (populated 153/789) is the highest-value instrumentation the vol lane emits. **Revisit trigger (registered C63):** re-opened by a future audit — never discretionarily — when post-change `vol_short` peer-excess reaches **≥ −5pp on n ≥ 40 decided rows across ≥ 2 regime buckets**. As with the short lane, that test only runs if short-vol theses keep being generated; **a session with zero short-vol candidates in a rich-vol tape is a screen to re-check, not a clean board.**


**Vol-lane class ceilings (2026-08-01 audit P1 #3).** `earnings_vol` caps at **0.55**, `high_iv_rank` at **0.60** — tightening-only, on top of the N-cap and the 0.80 absolute ceiling; tightest wins. Post-freeze these classes realise **0.449 (n=78)** and **0.562 (n=16)** while supplying **all 9 post-freeze ≥0.80 quotes**, pinned at exactly the ceiling. Levels pre-registered for re-grading; enforced at emission by `signal-confluence-quant.md`.
...
**2026-06-12 audit P2.1 — grade against the pre-committed daily envelopes, not a hindsight reconstruction.** The prior version reconstructed each name's "early-week direction" *at week-end* (when the outcome is already known) from a week-end-survivor universe ("tickers surfaced by Phase-1 agents this week"). Three biases, all inflationary, that this step now removes: (a) **hindsight direction** — the thesis direction is read from the call that was *written* early in the week, not re-inferred Friday; (b) **survivorship** — names whose Monday signal collapsed mid-week drop out of week-end persistence screens and silently leave the denominator; (c) **silent INCONCLUSIVE escape** — an undefined "insufficient movement" let losers be parked as INCONCLUSIVE.

**Universe (fixes survivorship):** the scorecard's universe is the **union of `calls[]` across this week's daily `decision.json` envelopes** (`analyses/daily/<covered_date>/decision.json`, non-DROP calls), keyed by `(ticker, direction)`. These are the calls as *committed at the time*, including ones whose thesis later collapsed — that is the point. Reading them is **explicitly permitted here** (it is the one place the weekly's "fresh-CLI, no daily files" principle is overridden, precisely because the envelopes are the only hindsight-free record of what was actually called). For any covered date with **no envelope** (daily wasn't run), fall back to a fresh-CLI reconstruction for that date only, and **tag those rows `reconstructed` (lower confidence, hindsight-exposed)** so the headline can report them separately.

**Thesis (hindsight-free):** take each call's `direction` and entry context from its envelope (the pre-committed record). Do NOT re-derive direction from Friday's flow.

**Resolution:** grade each call's outcome with fresh `uw` queries from its envelope date forward to `covered_dates[-1]`:
1. **Direction (primary)** — `uw historical trend` from the call's report_date → week-end: did the underlying move in the call's `direction`?
2. **Flow/OI corroboration** — `uw historical cumulative-premium-flow` + `uw historical oi-trend --days 5`: did net premium / OI build sustain or roll off in the thesis direction? (`uw hot-chains sweep-persistence` `sessions_in_top` is participation-only — pair with `smart-money-flow` for direction, per P1.5; never read a per-day directional series off it.)

**Grade — with a QUANTITATIVE INCONCLUSIVE threshold (fixes the silent escape):**
- **WIN** — the underlying moved ≥ **0.5 × ATR(14)** in the call's `direction` over the resolution window (the calibration-audit C20 path-aware threshold), corroborated by flow/OI direction.
- **LOSS** — moved ≥ 0.5×ATR(14) *against* the call's direction, OR flow decisively reversed against it.
- **INCONCLUSIVE** — |move| < 0.5×ATR(14) (genuinely no resolution yet). INCONCLUSIVE is **not** a dumping ground for contradictory signals — a name that moved ≥0.5×ATR against thesis is a LOSS, not INCONCLUSIVE.

Summarise in a table: `| Ticker | Direction | Source (envelope <date> / reconstructed) | Move vs ATR | Flow/OI | Grade | Note |`

**Report the full denominator (fixes the hidden-denominator finding).** The Executive-Summary **Signal performance** headline must read: `X of Y resolved (hit rate X/Y = Z%); W INCONCLUSIVE excluded; U total calls in universe (E from envelopes, R reconstructed)`. Hit rate = `wins / (wins + losses)`; surface the INCONCLUSIVE count and the total so a reader sees how much was excluded and whether the universe was envelope-anchored or hindsight-reconstructed. A high hit rate on a small resolved fraction of a large universe is a tell, not a triumph — show the fraction.
...
- **Signal performance:** <X of Y resolved (hit rate %); W INCONCLUSIVE excluded; U total in universe (E envelope-anchored, R reconstructed) — per Step 6, P2.1 denominator-transparent>
...
[Table: Ticker | Direction | Source (envelope <date> / reconstructed) | Move vs ATR | Flow/OI | Grade | Note. Per Step 6 (P2.1): universe = the week's daily `decision.json` `calls[]` union (hindsight-free, survivorship-free); thesis direction from the pre-committed envelope; INCONCLUSIVE = |move| < 0.5×ATR(14); headline reports the full denominator (resolved / INCONCLUSIVE / total, envelope vs reconstructed). Daily envelopes ARE read here — the one deliberate override of the fresh-CLI principle, since they are the only hindsight-free record of what was called.]
