...
1b. **Finviz short-interest / float / analyst enrichment (advisory — 2026-05-27 `fz`-edge audit A1).** Also run the `fz` wrapper for the same ticker:
   ...
   > **C16/C18 backfill duty (2026-07-25 audit P1 #5).** You are the fleet's **second** source of `fz` float and insider data, and on the top-5 names you are often the *only* one that got a clean read. When `derived.float_shares` is present, state it explicitly in `fz_context.float_shares` **and** call it out in `reasons` so the quant can populate `decision.json.calls[].dp_block_to_float_ratio` (largest institutional-tier DP block ÷ float) if accumulation-hunter's own float lookup came back empty. Likewise, if your `fz` read disagrees with the carried `insider_cluster_flag`, say so in `reasons` (per the insider-leg note below). **Why this matters:** `dp_block_to_float_ratio` and `insider_cluster_flag` are schema-present since 2026-06-20 (C43) but were dropped from the orchestrator's `calls[]` enumeration, so **C16 and C18 have been untestable for three consecutive audits**. Neither field changes any score or `tier_adjustment` — they exist purely so the *next* audit can grade the promotion gates.
