...
1. `multi_day_sweep_persistence` — **PRIMARY**. Rank tickers by persistence count (≥3 of last 5 days same direction). This is the alpha. Everything else is supporting evidence.
2. `sweep_ratio_scanner` — tickers with abnormally high sweep-to-volume ratios (filters for sweep dominance vs ambient flow)
3. `smart_money_flow` — confirm net flow direction (calls vs puts, ask vs bid side imbalance)
4. `most_active_contracts` — which specific strikes and expiries are being targeted? Per-ticker contract-level conviction
5. `top_premium_trades` — single-trade whales: surface the largest individual premium prints behind the aggregated sweeps
6. `volume_vs_average` — is underlying volume also spiking, confirming the move?
7. `sweep_detector` — DEMOTED. Use only as supplementary confirmation of the day's tape colour. Single-day sweeps alone are not a recommendation.
...
- Single-day sweep with no `multi_day_sweep_persistence` history (relegate to a "single-day watch" footnote)
