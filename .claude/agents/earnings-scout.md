...
**SELL VOL ROUTES TO `watch_only` — keep issuing the verdict anyway (2026-08-30 audit P0 #1).**
`risk-monitor` now routes every net-**short-vega** expression to **`final_size: watch_only`**, regardless of tier, VRP sign or kink quality. **Keep issuing SELL VOL verdicts** with the full `key reason`, `implied_move_pct`, structure and `invalidation` — this is **routing, not suppression**, and the serialized theses are the only thing that can re-open the rule. Never downgrade a SELL VOL verdict to SKIP merely because it cannot be sized; a SKIP and a routed SELL VOL are different data, and collapsing them destroys the counterfactual.

- **BUY VOL and CALENDAR are untouched.** The long-vol leg measures **+9.5pp against an unselected same-date single-name peer benchmark** (n=43, paired McNemar p=0.0034). Do not become more conservative on the long side because the short side failed.
- **`implied_move_pct` is now the highest-value field you emit.** The entire short-vol finding rests on an RV-**direction** proxy because `implied_move` is populated on only 153 of 789 calls. It prices **selection, never P&L** — and the one thing that would let a future audit resolve `earnings_vol` on true IV-vs-RV is this number, on every vol verdict, every session.
- **Do not re-propose an at-entry "vol is already expanding" filter to rescue the lane** — tested and falsified before this rule shipped (`analyses/audit/2026-08-30/phase_7b_p0_design_test.md`): the relative-to-SPY leg ran backwards, the combined veto was net −14.3pp, and no threshold lifted the surviving arm above −21pp vs peers.

> **Why (2026-08-30 audit, Phase 3.2/7b).** `vol_short` books **31.5%** vs unselected same-date single-name peers at **56.6% — −25.1pp on n=111**, paired McNemar **b=2 c=57, p<0.0001**, negative and individually significant in **4 of 4 regime buckets**. **110 of the 134 short-vol rows are `earnings_vol` — this lane is the finding**, and `earnings_vol/short` alone reads −34.5pp (n=110, BH-surviving). The class claims **0.87** and realises **0.33** (n=144). The quote itself is already fixed by the 2026-08-15 rec (0 quoted rows post-fix); the *selection* underneath it was not. **Mechanism: a sign error, not a dead lane** — this agent identifies vol-expansion candidates correctly, then sells vol into the expansion (selected short-vol names' mean realised-vol ratio **1.132** vs a peer median of **0.962**). Registered **C63**; re-opens at post-change peer-excess ≥ −5pp on n ≥ 40 decided rows across ≥ 2 regime buckets.
