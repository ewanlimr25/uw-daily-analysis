...
- **Market-excess gate (2026-05-25 register C2) — beta cannot masquerade as edge.** The N-cap controls the *magnitude* of the quote but not its *beta*: a signal measured purely in an up-tape can still clear the sizing line. So in addition to the cap, compute a **market-excess win-rate** and use it as a **downgrade-only gate** (it never raises size; the calibrated ladder still runs on the raw capped `win_rate`):
  - **benchmark** = the **same-direction** SPY bet over the **same signal windows** as the class's `historical_signal_backtest` (LONG class → fraction of those windows SPY rose; SHORT class → fraction SPY fell). Compute it from a **dedicated SPY `historical_trend`** over the same dates. **Do NOT** reuse SPY's appearance *inside* the signal class's own backtest set as the benchmark — that conflates the signal with its reference and double-counts SPY.
...
- **OI-confirmed-opening gate on the directional flow classes (2026-05-25 register C4) — only opening flow predicts.** When `dominant_signal_class` is `bullish_flow` or `bearish_flow`, the class's sizing is only trustworthy if the flow is **opening new positioning**, not closing / churning intraday (Pan & Poteshman 2006: the predictive option-volume signal is built *only* from buy-to-open volume; the edge disappears without isolating opening flow). Confirm opening from the **aggregate** OI:
  - `historical_oi_trend` BUILDING in the trade direction → confirmed; FALLING / rolling-off → not confirmed (closing-dominant).
  - else ΔOI (`oi_biggest_increases` / `oi_decrease_with_volume`) ≥ **20% of the day's contract volume** → opening-dominant.
...
**Conditional `dark_pool_accumulation` conjunction (2026-05-25 register C11).** The `+3 3+ aligned signals in accumulation-hunter (DP + OI + oi_smart_positioning, dark_pool_block_stratified institutional-tier confirmed)` rubric line is a **conjunction (interaction term), not an additive standalone.** Award the **full +3 only when `cum_premium_flow_30d` confirms** the accumulation direction — both gates true:
...
- **Lottery / expensive-skew `−2` fade (register C6) — WITHHELD.** Gated to **non-UPTREND regimes only** (the current regime is UPTREND) and `historical_pc_ratio_zscore` standalone backtested NEGATIVE (−22pp); the (d) bar (top-decile underperforms by ≥X% at p<0.10) is unmet on this data. `vol-surface-scout` may surface a `lottery_score` composite as advisory prose, routed through the C13 router (never stacked with `−2 overcrowded-long`); no scored line ships.
