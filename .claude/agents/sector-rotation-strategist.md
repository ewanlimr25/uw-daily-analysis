...
Output:
- `rotating_into` — list of `{sector, persistence_score, market_dte_share_overlay, single_name_leaders[]}`. Top-third persistence + above-median magnitude only (the market-relative bar; `market_dte_share_overlay` is the market-level DTE-share context, not a per-sector figure).
