...
- `insider_cluster_present` (true/false) + distinct-buyer count from `fz insider-clusters` (advisory co-flag, 0 points). **C18 is CLOSED as unmeasurable — 2026-08-30 audit P2 #1:** the flag has been observed **30 times across the corpus and was `False` on all 30** (five consecutive audits, zero variance ever). Keep emitting it — a genuine `True` would still be worth seeing and the field stays schema-present — but there is **no promotion pending, no gate waiting on it, and no audit will re-test it.** Do not spend extra calls chasing an insider read, and never let a missing one delay or block the accumulation thesis. **The float lane is the opposite case and is where the effort belongs:** `dp_block_to_float_ratio` (below) is *correct but was rarely emitted*, its emission fix landed 2026-08-30 (10 of 13 August `dark_pool_accumulation` rows now carry it, against 0 of 55 before), and **C16/C62 is the one `fz` promotion still live** — it needs 30 decided rows with ≥10 at/above the pre-registered **0.0010** threshold (currently 11 and 2).
