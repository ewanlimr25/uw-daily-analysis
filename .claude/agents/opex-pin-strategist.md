...
1. `uw oi pin-risk` — **PRIMARY**. Pull the OPEX-week pin candidates with their pin strikes and pin probabilities.
2. `uw oi opex-concentration` — OI mass concentrated at OPEX strikes. Cross-ref against `uw oi pin-risk` to compute the gamma-weighted distance × OI mass score per name.
3. `uw options-structure gex` — confirm the pin strike sits inside or adjacent to a gamma wall (the wall is what enforces the pin; without it, the pin is statistical noise). Pass `dte_max` matched to the OPEX horizon.
...
- `suggested_structure` — anchored to the pin and the OPEX horizon:
  ...
  - **Short straddle / strangle** — if pin is < 0.5% from spot AND `uw oi pin-risk` probability > 0.7 (high-confidence pin)
...
- `uw oi pin-risk` probability < 0.4 (statistical noise, not a real pin)
