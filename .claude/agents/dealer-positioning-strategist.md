...
1. `uw options-structure dex` (DEX) — **PRIMARY**. Net dealer delta, with sign. DEX flips (positive→negative or vice versa) are pre-directional signals; the price move follows the DEX flip with a 1–4 week lag. Always state the current DEX, the sign, and the trailing 5d/10d trajectory.
